...
Public Class frmMediddas
    Private tendenciaCentral As TendenciaCentral
    Private tipoDistribucion As String = String.Empty
    Private TendenciaCentralT As New TendenciaCentral()
    Public Sub New()
        InitializeComponent()
        tendenciaCentral = New TendenciaCentral() ' Inicialización de la clase
    End Sub
    ...
    Private Sub btnSimple_Click(sender As Object, e As EventArgs) Handles btnSimple.Click
        ...
        If txtNominal.Text.Trim() = "" Then
            MessageBox.Show("Por favor, ingresar Clase(X).", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning)
    ...
    Private Sub btnInvervalos_Click(sender As Object, e As EventArgs) Handles btnInvervalos.Click
        ...
        'llenar texbox
        Dim resultadoColumna1 As New StringBuilder()
        Dim resultadoColumna2 As New StringBuilder()
        Dim resultadoColumna3 As New StringBuilder()
    ...
    Private Sub btnCentral_Click(sender As Object, e As EventArgs) Handles btnCentral.Click
        Try
            ...
            ' Suponiendo que tienes un TextBox donde el usuario ingresa los datos
            Dim datosString As String = txtNominal.Text.Trim()


            ' Verificar si el TextBox está vacío
            If String.IsNullOrEmpty(datosString) Then
                MessageBox.Show("Por favor, ingresa los datos.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Return
            End If

            ' Separar los datos en un array usando espacio como delimitador
            Dim datos() As String = datosString.Split(" "c)
            Dim cantidadDatos As Integer = datos.Length
            ' Crear un array para los datos en formato Double
            Dim datosNumericos(cantidadDatos - 1) As Double

            ' Convertir los datos a Double
            For i As Integer = 0 To cantidadDatos - 1
                Dim valor As String = datos(i).Trim()

                ' Verificar si el valor es numérico antes de convertir
                If Not Double.TryParse(valor, datosNumericos(i)) Then
                    MessageBox.Show($"El valor '{valor}' no es numérico. Por favor, ingresa solo números.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Return
                End If
            Next
            ...
            If tipoDistribucion = "SerieSimple" Then
                ...
                resultado = tc.MediaAritmetica(cantidadDatos, datosNumericos)
                txtMedia.Text = resultado.ToString("F2") ' Mostrar en TextBox correspondiente
                resultado = tc.CalcularMediana(cantidadDatos, datosNumericos)
                txtMediana.Text = resultado.ToString("F2") ' Mostrar en TextBox correspondiente
                ' Calcular moda para series simples
            ...
            ElseIf tipoDistribucion = "DatosAgrupados" Then
                Try
                    ...
                    ' Llama a la función para calcular la media aritmética
                    ...
                    ' txtMedia.Text = resultado.ToString("F2")
    ...
    Private Sub btnComentarioCentral_Click(sender As Object, e As EventArgs) Handles btnComentarioCentral.Click
        ...
        If txtMedia.Text.Trim() = "" Then
            MessageBox.Show("Por favor, calcule los resultados para generar comentario", "Validación", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Return ' Salir del método si el TextBox está vacío
        End If
    ...
    Private Sub btnDispersion_Click(sender As Object, e As EventArgs) Handles btnDispersion.Click
        Try
            ...
            ' Verificar si el TextBox está vacío
            If txtMedia.Text.Trim() = "" Then
                MessageBox.Show("Por favor, el promedio (media artimetica).", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Return ' Salir del método si el TextBox está vacío
            End If
    ...
    Private Sub frmMediddas_Load(sender As Object, e As EventArgs) Handles MyBase.Load
